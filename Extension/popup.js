...
document.addEventListener('DOMContentLoaded', function() 
{
    ...
    //settings set
    document.getElementById('wrap_settings').addEventListener('click', function() {
    ...
    document.getElementById('scan-btn').addEventListener('click', function() 
    {
        ...
        chrome.tabs.query({active: true, currentWindow: true}, function(tabs) 
        {
            ...
            if (currentTab && currentTab.url) 
            {
                ...
                chrome.runtime.sendMessage({from: 'popup', subject: 'scan-page', url: currentTab.url}, (response) => 
                {
    ...
    document.getElementById('report-error-btn').addEventListener('click', function() {
        chrome.tabs.query({active: true, currentWindow: true}, function(tabs) 
        {
            ...
            chrome.tabs.sendMessage(tabs[0].id, {action: "openModal"}, function(response) {
                if (chrome.runtime.lastError) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (reportData) {
        ...
        document.getElementById('company').textContent = reportData.company ?? "No Company Name";
        document.getElementById('company-name').textContent = reportData.company ?? "No Company Name";
        document.getElementById('category').textContent = reportData.category ?? "No Category";
        ...
        document.getElementById('report-btn').addEventListener('click', function() 
        {
            ...
            chrome.storage.local.get('documentID', (result) => {
                ...
                if (documentID) {
                    chrome.runtime.sendMessage({ type: "FETCH_REPORT", documentID }, () => {
                        console.log("Tracking backend task for document ID:", documentID);
                    });
                } else {
...
function updateRiskData(riskAssessment) {
    ...
    if (Array.isArray(riskAssessment)) {
        ...
        console.error("Invalid data structure, expected array or object. Got:", typeof riskAssessment);
    }
}
...
function updateRiskFields(risk) {
    ...
    if (risk.Legal) {
        ...
        document.getElementById('legal-risk-level').textContent = getRiskLevelLabel(legalScore);  // Update Legal Risk Label
    }

    if (risk.Financial) {
        const financialScore = parseInt(risk.Financial.Score) || 0;
        updateCircleProgress('privacy-risk', financialScore);
        document.getElementById('financial-risk-level').textContent = getRiskLevelLabel(financialScore);  // Update Financial Risk Label
    }

    if (risk.Data) {
        const dataScore = parseInt(risk.Data.Score) || 0;
        updateCircleProgress('data-risk', dataScore);
        document.getElementById('data-risk-level').textContent = getRiskLevelLabel(dataScore);  // Update Data Risk Label
    }
...
function updateReadability(score) {
    ...
    fillElement.style.transition = 'width 1s ease-in-out, background-color 1s ease-in-out';
...
function getDynamicColor(score) {
    ...
    if (percentage <= 0.5) {
        ...
        r = Math.round(easyColor.r + (moderateColor.r - easyColor.r) * midPercentage);
        g = Math.round(easyColor.g + (moderateColor.g - easyColor.g) * midPercentage);
        b = Math.round(easyColor.b + (moderateColor.b - easyColor.b) * midPercentage);
    } else {
        const midPercentage = (percentage - 0.5) * 2;
        r = Math.round(moderateColor.r + (challengingColor.r - moderateColor.r) * midPercentage);
        g = Math.round(moderateColor.g + (challengingColor.g - moderateColor.g) * midPercentage);
        b = Math.round(moderateColor.b + (challengingColor.b - moderateColor.b) * midPercentage);
    }
    return `rgb(${r}, ${g}, ${b})`;
}
...
function updatePopupContent(receivedData) 
{
    return new Promise((resolve, reject) => {
        ...
        .then(data => {
            ...
            document.getElementById('company').textContent = receivedData.company ?? "No Company Name";
            document.getElementById('company-name').textContent = receivedData.company ?? "No Company Name";
            document.getElementById('category').textContent = receivedData.category ?? "No Category";
            document.getElementById('date').textContent = receivedData.date ?? "No Date";
                        ...
                        chrome.runtime.sendMessage(
                            ...
                            (response) => {
                                if (response.success) {
                                    ...
                                    console.error("Failed to fetch document data:", response.message);
                                }
                            }
                        );
                    } else {
        ...
        .catch(error => {
            ...
            document.getElementById('status-update').textContent = "Failed to load the document data.";
            reject();
        });
    });
}
