...
function ReportLibrary() {
  ...
  function toLowerCaseCustom(input) {
  ...
  const filteredDocuments = useMemo(() => {
    ...
    if (searchQuery) {
      docs = docs.filter(
        (doc) =>
          toLowerCaseCustom(doc.companyName).includes(
            toLowerCaseCustom(searchQuery)
          ) ||
          toLowerCaseCustom(doc.documentType).includes(
            toLowerCaseCustom(searchQuery)
          )
      );
    }
      ...
      try {
        ...
        const documentsWithRisk = await Promise.all(
          data.map(async (doc) => {
            ...
            console.log(
              "docID is " + doc.DocumentID + "Flag data:" + flagData.flag
            );
