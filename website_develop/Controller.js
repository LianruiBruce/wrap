...
async function processData(data) {
  ...
  try {
    const response = await axios.post("https://localhost:5000/process-url", {
      data,
    });
...
async function generateReport(data) {
  try {
    ...
    const response = await axios.post(
      "https://localhost:5000/generate-report",
      {
        data,
      }
    );
...
async function generateSections(data) {
  try {
    ...
    const response = await axios.post(
      "https://localhost:5000/generate-sections",
      {
        data,
      }
    );
...
async function generateTitleReportByPDF(text) {
  try {
    ...
    const response = await axios.post("https://localhost:5000/get-PDF-info", {
      text,
    });
    console.log(
...
async function generateReportByPDF(text, userSettings, io, userID) {
  try {
    ...
    if (oldDocument) {
      ...
      const response = await axios.post(
        "https://localhost:5000/generate-reportByPDF",
        { text: text, userSettings: userSettings }
      );
